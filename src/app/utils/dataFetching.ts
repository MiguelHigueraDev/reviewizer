...
export const fetchReviews = async (
  ...
  try {
    const response = await fetch(
      `${REVIEWS_URL}${appId}?use_review_quality=0&cursor=*&day_range=30&start_date=-1&end_date=-1&date_range_type=all&filter=all&language=english&l=english&review_type=all&purchase_type=all&playtime_filter_min=0&playtime_filter_max=0&filter_offtopic_activity=1&summary_num_positive_reviews=959749&summary_num_reviews=1110039&json=1`
    );
